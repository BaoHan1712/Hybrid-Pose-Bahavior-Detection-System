...
# Tạo dictionary để lưu trữ lịch sử nhãn cho mỗi bbox
frame_count = 0
bbox_history = {}  # {bbox_id: {'labels': [], 'predictions': []}}

# Hàm để tính toán nhãn phổ biến nhất trong 10 frame
def get_most_common_label(labels):
    if not labels:
        return None
    working_count = sum(1 for label in labels if label == "working")
    not_working_count = len(labels) - working_count
    return "working" if working_count > not_working_count else "not_working"
...
while cap.isOpened():
    ...
    results = model_pose.predict(source=frame, imgsz=640, conf=0.3, verbose=False)

    frame_count += 1
    current_boxes = set()  # Lưu các bbox trong frame hiện tại
    ...
    for result in results:
        ...
        for box, keypoint in zip(boxes, keypoints):
            ...
            if point_in_polygon((center_x, center_y), region_points) and conf > 30:
                # Tạo bbox_id dựa trên tọa độ trung tâm
                bbox_id = f"{center_x}_{center_y}"
                current_boxes.add(bbox_id)
                ...
                if cropped_image.size > 0:
                    ...
                    with torch.no_grad():
                        ...
                        current_label = "working" if predicted.item() == 1 else "not_working"

                    # Cập nhật lịch sử nhãn
                    if bbox_id not in bbox_history:
                        bbox_history[bbox_id] = {'labels': [], 'predictions': []}
                    
                    bbox_history[bbox_id]['labels'].append(current_label)
                    
                    # Giữ chỉ 20 frame gần nhất
                    if len(bbox_history[bbox_id]['labels']) > 20:
                        bbox_history[bbox_id]['labels'] = bbox_history[bbox_id]['labels'][-20:]

                    # Cập nhật dự đoán mỗi 20 frame
                    if frame_count % 20 == 0:
                        final_label = get_most_common_label(bbox_history[bbox_id]['labels'])
                        bbox_history[bbox_id]['predictions'].append(final_label)
                    
                    # Hiển thị nhãn
                    display_label = (bbox_history[bbox_id]['predictions'][-1] 
                                   if bbox_history[bbox_id]['predictions'] 
                                   else current_label)
    ...
    # Xóa lịch sử của các bbox không còn xuất hiện
    bbox_ids = list(bbox_history.keys())
    for bbox_id in bbox_ids:
        if bbox_id not in current_boxes:
            del bbox_history[bbox_id]
